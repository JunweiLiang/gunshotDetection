...
static int initGDFSAItf(ClientData cd, int argc, char *argv[]){
  ...
  if(itfParseArgv(argv[0], &argc, argv, 1,
    ...
    "-mmi_init", ARGV_INT, NULL, &mmi_init, "", "E",
    NULL) != TCL_OK) return TCL_ERROR;

  if(!use_fsa)
    gdfsa = new GDFSA(senone_set, mladapt, codebook_set, base_codebook_set, distrib_set, feature_set, svmap, false, scale, ref_lock, E, mmi_init);
  else
    gdfsa = new GDFSA(senone_set, mladapt, codebook_set, base_codebook_set, distrib_set, feature_set, svmap, true, scale, ref_lock, E, mmi_init);
