  ...
  for(int i=0;i<glatP->nodeN;i++){
    ...
    for(int parentX=0;parentX<nodeP->addon.parentCtxN;parentX++){
      for(int childX=0;childX<nodeP->addon.childCtxN;childX++){
        ...
        fprintf(stderr, "Score: %e\n", nodeP->addon.score[parentX][childX]);
...
void MPE::glatBMMIAdjustScores(GLat* glatP, float boost_factor, float scale){
  float norm = 1.0/scale;
  ...
  for(int i=0;i<glatP->nodeN;i++){
    ...
    for(int j=0;j<MPECompP->monoPhoneN;j++){
      offset += MPECompP->approxPhoneAcc[j];
    }
    nodeP->score +=norm*boost_factor*offset;
    for(int parentX=0;parentX<nodeP->addon.parentCtxN;parentX++){
      for(int childX=0;childX<nodeP->addon.childCtxN;childX++){
        score[parentX][childX] += norm*boost_factor*offset;
      }
    }
  }
}
