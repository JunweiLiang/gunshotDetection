...
GDFSA::GDFSA(SenoneSet* sns, CodebookSet* cbs, DistribSet* dss, FeatureSet* fs, SVMap* svmap, int num_classes, bool use_fsa, float scale, int ref_lock, float E, int mmi_init){
  this->sns = sns;
  this->cbs = cbs;
  this->dss = dss;
  this->fs = fs;
...
void GDFSA::kmeanCluster(){
  Matrix mean(num_classes, dim);
...
/*
  for(int r=0;r<num_classes;r++){
    ...
    for(int d=0;d<dim;d++){
      mean[r][d] /= (float) count;
    }
  }
*/
  count.resize(num_classes);
  for(int i=0;i<cbs->list.itemN;i++){
    Codebook* cb = cbs->list.itemA + i;
    for(int m=0;m<cb->refN;m++){
      int r = rand()%num_classes;
      for(int d=0;d<dim;d++){
        mean[r][d] += cb->rv->matPA[m][d];
      }
      assignment.push_back(r);
  ...
  for(int j=0;j<5;j++){
    ...
    for(int i=0;i<cbs->list.itemN;i++){
      ...
      for(int m=0;m<cb->refN;m++){
        ...
        for(int r=0;r<num_classes;r++){
          float dist = 0.0;
          for(int d=0;d<dim;d++){
            dist += (mean[r][d] - cb->rv->matPA[m][d])*(mean[r][d] - cb->rv->matPA[m][d]);
          }
          if(r==0 || dist < min_dist){
            assignment[idx] = r;
            min_dist = dist;
          }
        }
    ...
    for(int r=0;r<num_classes;r++){
      if(count[r]==0)
        continue;
//      fprintf(stderr, "iter %d, cluster %d: %d\n", j+1, r, count[r]);
      for(int d=0;d<dim;d++){
        mean[r][d] /= (float)count[r];
      }
    }
  ...
  for(int r=0;r<num_classes;r++){
    Gaussian* g = cluster[r];
    g->setMeanVec(mean.getRowVec(r));
    g->setCovMat(cov);
    g->calculateGConst();
  }
}

void GDFSA::initCluster(){
  cluster.clear();
