...
void MathEngine::svd(const Matrix& a, Matrix& u, Matrix& s, Matrix& v){
  char jobu='A', jobvt='A';
  long long m = a.getNumRow();
  long long n = a.getNumCol();
...
void MathEngine::deconSVD(const Matrix& a, Matrix& u, Matrix& s, Matrix& v){
  ...
  long long ldvt=min;
  long long lwork=3*min*min+max+4*min*min+4*min;
  double* work = new double[lwork];
  long long* iwork = new long long[8*min];
  long long info=0;
...
void MathEngine::eigenDecomp(const Matrix& m, Matrix& eigen_values, Matrix& eigen_vectors){
  ...
  fprintf(stderr, "%d %d %d\n", n, sizeof(integer), sizeof(long long));
