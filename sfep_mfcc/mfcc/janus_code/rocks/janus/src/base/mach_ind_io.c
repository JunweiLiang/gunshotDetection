/* ========================================================================
    JANUS      Speech- to Speech Translation
               ------------------------------------------------------------
               Object: Machine Independent IO
               ------------------------------------------------------------

    Author  :  Arthur McNair & Tilo Sloboda
    Module  :  machIndIO.c
    Date    :  $Id: mach_ind_io.c 3241 2010-05-17 18:15:19Z metze $
    Remarks :
   ...
   ========================================================================
    ...
    Revision 3.5.4.1  2002/07/18 12:19:26  soltau
    Removed malloc.h

    Revision 3.5  2001/01/15 09:49:59  janus
    Merged Christian's version with WINDOWS support (branch jtk-00-11-14-fuegen)

    Revision 3.4.38.1  2001/01/12 15:16:57  janus
    necessary changes for running janus under WINDOWS

    Revision 3.4  2000/08/16 11:22:40  soltau
    *** empty log message ***

    Revision 3.3  1997/07/18 15:50:55  tschaaf
    DFKI-Clean
    set constants in swat ... tu to unsigned
...
#else
UBYTE float_to_ubyte( f ) float f;
#endif
{
  if (f < 0.0) 
...
#else
float ubyte_to_float( u ) UBYTE u;
#endif
{
...
#else
int read_floats( fp, whereto, count ) FILE *fp; float *whereto; int count; 
#endif
{
...
#else
int read_floatbytes( fp, whereto, count )  FILE *fp; float *whereto; int count;
#endif
{
...
#else
float read_floatbyte( fp ) FILE *fp;
#endif
{
  UBYTE    b;
  float     val;
...
#endif
{
  ...
  switch (machine) {
    case RT_SUN: return(i); 			        /* RT or SUN:  already in correct format */

    case DEC:
    case VAX:    return( REVERSE_4BYTE(i) );		/* DEC or VAX:  reverse the bytes */
...
#else
int read_ints( fp, whereto, count ) FILE *fp; int *whereto; int count;
#endif
{
    union unionT v,u;		        /* union variables for converting values */
...
#endif
{
  ...
  switch (machine) {
    case RT_SUN: return(s); 			/* RT or SUN:  already in correct format */

    case DEC:
    case VAX:    return(SWAP_SHORT(s));	        /* DEC or VAX:  reverse the bytes */
...
#else
int read_shorts(fp, whereto, count) FILE *fp; short *whereto; int count;
#endif
{
...
#else
int read_string(fp, str) FILE *fp; char *str;
#endif
{
  int    status;
  short  len;
...
#else
void write_float (fp, f) FILE *fp; float f;
#endif
{
    union unionT u,v;
...
#else
int write_floats (fp, wherefrom, count) FILE *fp; float *wherefrom; int count;
#endif
{
    union unionT u,v;
...
#else
void write_floatbyte (fp, f) FILE *fp; float f;
#endif
{
     UBYTE  b;
...
#else
int write_floatbytes(fp, wherefrom, count) FILE *fp; float *wherefrom; int count;
#endif
{
    UBYTE  *tmpA;			/* temporary array to hold all data for conversion */
...
#else
void write_int( fp, i) FILE *fp; int i;
#endif
{
    union unionT u;
...
#else
int write_ints(fp, wherefrom, count) FILE *fp; int *wherefrom; int count;
#endif
{
...
#else
void write_short(fp, s) FILE *fp; short s;
#endif
{
     short  t;
...
#else
int write_shorts(fp, wherefrom, count) FILE *fp; short *wherefrom; int count;
#endif
{
...
#else
int short_memorychange(buf,bufN) short *buf; int bufN;
#endif
{
  switch(machine){
  case RT_SUN:      break;                              /* RT or SUN:  already in correct format */
...
#else
int int_memorychange(buf,bufN) int *buf; int bufN;
#endif
{
  int i,temp;
...
#else
int float_memorychange(buf,bufN) float *buf; int bufN;
#endif
{
  int i;
  union unionT u,v;
...
/*=============================================================================================================================
 ...
 +===========================================================================================================================*/
#ifdef __STDC__
int write_string(FILE *fp, char *str)
#else
int write_string(fp, str) FILE *fp; char *str;
#endif
{
     int    status;
     short  len;
...
#else
int write_scaled_vectors(fp, wherefrom, coeffN, vectorN) FILE *fp; float *wherefrom; int coeffN; int vectorN;
#endif
{
...
#else
int read_scaled_vectors( fp, whereto,  coeffNP, vectorNP )
    FILE *fp; float **whereto;  int *coeffNP; int *vectorNP;
#endif
{
...
#else
int read_scaled_vectors_range( fp, whereto,  coeffNP, vectorNP, from, to )
    FILE *fp; float **whereto;  int *coeffNP; int *vectorNP; int from, to;
#endif
{
  MIN_MAX *channelA;          /* malloc'ed array for the min/max values for each dimension */
  float   *deltaA;            /* malloc'ed array for the interval widths for each dimension */
...
#else
int set_machine(new_machine) int new_machine;
#endif
{
...
#else
int check_byte_swap( buf, bufN ) short *buf; int bufN;
#endif
{
    int  sum_dif_norm = 0;
    int  sum_dif_swap = 0;
...
#else
void buf_byte_swap( buf, bufN ) short *buf; int bufN;
#endif
{
